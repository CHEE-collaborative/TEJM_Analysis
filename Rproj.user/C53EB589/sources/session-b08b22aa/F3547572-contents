...
if(!dir.exists(file.path(data_path, "tmax"))){dir.create(file.path(data_path, "tmax"))}

# create a HICDD folder if it doesnt exist already
if(!dir.exists(file.path(data_path, "vp"))){dir.create(file.path(data_path, "vp"))}
...
GetSatVP <- function(t) {
  ifelse(t > 0, exp(34.494 - (4924.99/(t + 237.1)))/(t + 105)^1.57, exp(43.494 - (6545.8/(t + 278)))/(t + 868)^2)
}

# functions to calculate heat index 
hi <- function(t) -42.379 + 2.04901523 * t + 10.14333127 * rh - 
...
GetHeatIndex <- function (t = NA, rh = NA) { 
  ifelse(is.na(rh) | is.na(t) | is.nan(rh) | is.nan(t), NA,
         ifelse(t <=40, t,  
                ifelse(-10.3 + 1.1 * t + 0.047 * rh < 79, -10.3 + 1.1 * t + 0.047 * rh,
                       ifelse(rh <= 13 & t >= 80 & t <= 112, -42.379 + 2.04901523 * t + 10.14333127 * rh - 
                                0.22475541 * t * rh - 6.83783 * 10^-3 * t^2 - 
                                5.481717 * 10^-2 * rh^2 + 1.22874 * 10^-3 * t^2 * 
                                rh + 8.5282 * 10^-4 * t * rh^2 - 1.99 * 10^-6 * 
                                t^2 * rh^2 - (13 - rh)/4 * ((17 - abs(t - 95))/17)^0.5,
                              ifelse(rh > 85 & t >= 80 & t <= 87, -42.379 + 2.04901523 * t + 10.14333127 * rh - 
                                       0.22475541 * t * rh - 6.83783 * 10^-3 * t^2 - 
                                       5.481717 * 10^-2 * rh^2 + 1.22874 * 10^-3 * t^2 * 
                                       rh + 8.5282 * 10^-4 * t * rh^2 - 1.99 * 10^-6 * 
                                       t^2 * rh^2 + 0.02 * (rh - 85) * (87 - t), -42.379 + 2.04901523 * t + 10.14333127 * rh - 
                                       0.22475541 * t * rh - 6.83783 * 10^-3 * t^2 - 
                                       5.481717 * 10^-2 * rh^2 + 1.22874 * 10^-3 * t^2 * 
                                       rh + 8.5282 * 10^-4 * t * rh^2 - 1.99 * 10^-6 * 
                                       t^2 * rh^2)))))
...
DownloadDaymetData <- function(years, location) {
  ...
  # Creating folders for the parameters we want, which are maximum temperature and vapor pressure
  tmax_folder <- file.path(data_path, "tmax")
  vp_folder <- file.path(data_path, "vp")
  
  if (!dir.exists(tmax_folder)) 
    dir.create(tmax_folder, recursive = TRUE)
  
  if (!dir.exists(vp_folder))
    dir.create(vp_folder, recursive = TRUE)
  
  # Remove years that already exist in tmax_folder
  existing_tmax_files <- list.files(tmax_folder, pattern = ".nc$", full.names = TRUE)
  ...
  existing_vp_files <- list.files(vp_folder, pattern = ".nc$", full.names = TRUE)
  ...
  if (length(years) > 0) {
    ...
    download_daymet_ncss(location = location,
                         start = years[1],
...
GetHicddsPerYear_monthly <- function(year,census_data){
  ...
  hicdds_year <- lapply(1:5, function(i){
    ...
    cooling_season_start_index <- as.numeric(as.Date(paste0(year,cooling_season_starts[i])) - year_start_date)
    cooling_season_ends_index <- as.numeric(as.Date(paste0(year,cooling_season_ends[i])) - year_start_date)
    days_in_season <- cooling_season_ends_index - cooling_season_start_index + 1  
    
    # opening tmax data
    tmax <- nc_open(file.path(data_path, "tmax", paste0("tmax_daily_", year, "_ncss.nc")))
    tmax_array <- ncvar_get(tmax, "tmax")
    ...
    tmax_array_cooling_season <- tmax_array[, , cooling_season_start_index:cooling_season_ends_index]
    
    # calculating vapor pressure data corresponding to cooling season
    satVP_array_cooling_season <- GetSatVP(tmax_array_cooling_season[, , seq_along(1:days_in_season)])
    
    # removing tmax_array from memory to save space
    rm(tmax_array)
    
    # opening vapor pressure data
    vp <- nc_open(file.path(data_path, "vp", paste0("vp_daily_", year, "_ncss.nc")))
    long_vp <- ncvar_get(vp, "x")
    lat_vp <- ncvar_get(vp, "y")
    vp_array <- ncvar_get(vp, "vp")
    
    # fill missing values in with 0
    fillvalue <- ncatt_get(vp, "vp", "_FillValue")
    vp_array[vp_array == fillvalue$value] <- 0
    
    # extracting vapor pressure data corresponding to cooling season
    vp_array_cooling_season <- vp_array[, , cooling_season_start_index:cooling_season_ends_index]
    
    # removing vp_array from memory to save space
    rm(vp_array)
    
    # converting temperature values from Celsius to Fahrenheit
    tmax_array_cooling_season_F <- C_to_F(tmax_array_cooling_season[, , seq_along(1:days_in_season)])
    
    # calculating relative humidity
    relative_humidity <- lapply(1:days_in_season, function(j) (vp_array_cooling_season[, , j] / satVP_array_cooling_season[, , j]) * 100)
    
    # storing the relative humidity values in an array
    relative_humidity <- array(unlist(relative_humidity), dim = c(854, 811, days_in_season))
    
    # calculating heat index
    heat_index <- sapply(1:days_in_season, function(j) GetHeatIndex(t = tmax_array_cooling_season_F[, , j], rh = relative_humidity[, , j]))
    ...
    cdds_per_day <- lapply(1:days_in_season, function(j) CalcTempAboveThresh(heat_index[, , j]))
    ...
    for (i in 1:days_in_season) {
      step1 <- Reduce(`+`, lapply(list(cdds_per_day[[i]], cdds_across_summer), function(x) {
        x[is.na(x)] <- 0
        x
      }))
    ...
    # converting cooling degree data for the season into a raster and setting CRS
    raster_1year <- raster(t(cdds_across_summer), xmn = min(long_vp), xmx = max(long_vp), ymn = min(lat_vp),
                           ymx = max(lat_vp), crs = CRS("+proj=lcc +lat_1=25 +lat_2=60 +lat_0=42.5 +lon_0=-100 +x_0=0 +y_0=0 +ellps=WGS84 +units=km +no_defs"))
    raster_1year_epsg4326 <- projectRaster(from = raster_1year, crs = "+init=epsg:4326")
    
    # finding the CDDs per census area per year using exact_extract function
    tract_mean_cdds_1year <- exact_extract(raster_1year_epsg4326, census_data, weights = "area", fun = "mean", append_cols = TRUE) %>%
      mutate(year = year)
...
calculateHICDD <- function(years, census_data, observations = 1794){
  ...
  hicdds <- lapply(years, function(i){GetHicddsPerYear_monthly(year = i, census_data = census_data)})
  ...
  for (i in 1:length(years)){
    temp <- bind_rows(hicdds[[i]]) %>% mutate(month = month_var)
    hicdds_collection <- bind_rows(hicdds_collection, temp)
  } 
